...
struct base_field {
  ...
  static constexpr DEVICE_FORCEINLINE base_field two() { return base_field(2); }
  static constexpr DEVICE_FORCEINLINE base_field minus_one() { return base_field(MINUS_ONE); }
  constexpr base_field() = default;
  explicit constexpr DEVICE_FORCEINLINE base_field(const uint32_t limb) : limb(limb) {}
  ...
  DEVICE_FORCEINLINE thread base_field &operator+=(const base_field rhs) {
    *this = add(*this, rhs);
    return *this;
  }
  DEVICE_FORCEINLINE thread base_field &operator-=(const base_field rhs) {
...
struct alignas(8) ext2_field {
  base_field coefficients[2];
  static constexpr DEVICE_FORCEINLINE base_field non_residue() { return base_field::minus_one(); }
  static constexpr DEVICE_FORCEINLINE ext2_field zero() { return ext2_field(base_field::zero(), base_field::zero()); }
  static constexpr DEVICE_FORCEINLINE ext2_field one() { return ext2_field(base_field::one(), base_field::zero()); }
  constexpr ext2_field() = default;
  explicit constexpr DEVICE_FORCEINLINE ext2_field(const base_field c0, const base_field c1) : coefficients{c0, c1} {}
  explicit constexpr DEVICE_FORCEINLINE ext2_field(const thread base_field c[2]) : coefficients{c[0], c[1]} {}
  DEVICE_FORCEINLINE thread base_field &operator[](const unsigned idx) { return coefficients[idx]; }
  DEVICE_FORCEINLINE const thread base_field &operator[](const unsigned idx) const { return coefficients[idx]; }
  DEVICE_FORCEINLINE const thread base_field &base_coefficient_from_flat_idx(const unsigned idx) const { return coefficients[idx]; }
  ...
  template <class T> DEVICE_FORCEINLINE thread ext2_field &operator+=(const T rhs) {
    *this = add(*this, rhs);
    return *this;
  }
  template <class T> DEVICE_FORCEINLINE thread ext2_field &operator-=(const T rhs) {
...
struct alignas(16) ext4_field {
  ext2_field coefficients[2];
  static constexpr DEVICE_FORCEINLINE ext2_field non_residue() { return ext2_field(base_field::two(), base_field::one()); }
  static constexpr DEVICE_FORCEINLINE ext4_field zero() { return ext4_field(ext2_field::zero(), ext2_field::zero()); }
  static constexpr DEVICE_FORCEINLINE ext4_field one() { return ext4_field(ext2_field::one(), ext2_field::zero()); }
  constexpr ext4_field() = default;
  explicit constexpr DEVICE_FORCEINLINE ext4_field(const thread base_field c[4]) : coefficients{ext2_field(c[0], c[1]), ext2_field(c[2], c[3])} {}
  explicit constexpr DEVICE_FORCEINLINE ext4_field(const thread ext2_field c[2]) : coefficients{c[0], c[1]} {}
  explicit constexpr DEVICE_FORCEINLINE ext4_field(const ext2_field c0, const ext2_field c1) : coefficients{c0, c1} {}
  DEVICE_FORCEINLINE thread ext2_field &operator[](const unsigned idx) { return coefficients[idx]; }
  DEVICE_FORCEINLINE const thread ext2_field &operator[](const unsigned idx) const { return coefficients[idx]; }
  DEVICE_FORCEINLINE const thread base_field &base_coefficient_from_flat_idx(const unsigned idx) const { return coefficients[(idx & 2) >> 1][idx & 1]; }
  ...
  template <class T> DEVICE_FORCEINLINE thread ext4_field &operator+=(const T rhs) {
    *this = add(*this, rhs);
    return *this;
  }
  template <class T> DEVICE_FORCEINLINE thread ext4_field &operator-=(const T rhs) {
